## Notes from Terence - Lead Developer
This branch is set up to utilize Okta sign-in feature, however it does not play well with GitHub Pages/ Heroku/ Netify due to it using BrowserRouter (the GitHub Pages/ Master version uses HashRouter to work properly). This branch will work well on actual hosting services, though. Needs SSL for log in to work (read on for more on that).

Having a lot of experience with ReactJS, I decided to start this project off with that structure in mind. The major hurdles that have kept our project from launching by the beginning of 2018 is the authorization (Sign Up, Log In, Log Out) and marketplace. Luckily, I found an authorization/membership service from Okta that handles the database of users for us (up to 1000 members - free service). However, we would still need a back-end server portion of the website built to handle how authorization is used on the website, as I understand it. Also, for the marketplace, where listings are uploaded, registered, and paid for, I haven't gotten to figuring out all the requirements yet (as of Jan, 3rd). I do know that we'll need an SSL certificate, which is what encrypts our website to allow for safe and secure browsing and handling of information. For the SSL certificate, there's an application form and possibly payment for it. For example, with a web hosting service like Host Gator, we'd need to get at least the Web Hosting "Baby Plan" and then apply for an SSL certification once we have everything for the application. Check at www.hostgator.com.

Other options that I explored for a short time: Using Wordpress CMS (Content Management System) is very popular and has many plug-ins that help run all the above stuff. However, the thing with that is that many plug-ins require payment and some require subscription. For example, WooCommerce Vendors plug-in costs around $79 (one time payment), but WooCommerce Subscriptions (possibly necessary in the future) is an annual subscription at about $100 per year. Also, themes that can help build out our site tend to cost money as well. There are two that I found that seem to be able to handle what we want to do: Listify and Listable (both are from different companies). Listify has a lot of features, support, and good reviews and is about $79. It would provide a foundation to build the website with, including styling, layout, and many plug-ins. Listable is similar, but with less features. I found downloads for both and tried them out (I didn't pay - just downloaded to test them out) and Listify worked better but had some major issues that kept it from being easy to set up. That may be due to it not being an official version. If the time comes to try this route, I would attempt Listify first. This route allows for non-programmers to work on it as well.
...
## Notes for the overall project
  ...
  - To see the code of the authentication enabled build, visit the okta-run branch. A temporary live build is available on buddysjapan.com.
...
- Login and register. OAuth2. - High
  ...
  - 12/28, discovered HashRouter does not work with Okta - switched to BrowserRouter, solved spinning login process
  - 12/31, solved Okta authentication structure issue. App.js now passes Security and related functions, Main.js handles app state and authentication state and props.
